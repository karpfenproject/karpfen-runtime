...
class EnvironmentThread(
    ...
    private fun buildSimpleMessage(entry: EngineTraceLogger.TraceEntry): String {
        ...
        return when (entry.eventType) {
            ...
            EngineTraceLogger.TraceEventType.TRANSITION_START -> "${d["from"] ?: "?"} -> ${d["to"] ?: "?"}"
            ...
            EngineTraceLogger.TraceEventType.SPLIT_START -> "split: ${d["from"] ?: "?"} -> ${d["to"] ?: "?"}"
            EngineTraceLogger.TraceEventType.SPLIT_FIRED -> "split: ${d["from"] ?: "?"} -> ${d["to"] ?: "?"}"
            EngineTraceLogger.TraceEventType.JOIN_START -> "join: ${d["from"] ?: "?"} -> ${d["to"] ?: "?"}"
